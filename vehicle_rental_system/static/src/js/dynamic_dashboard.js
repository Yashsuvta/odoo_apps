...
export class OdooDynamicDashboard extends Component {
    ...
    setup() {
        ...
        onWillStart(async () => {
            ...
            await loadJS("https://ajax.googleapis.com/ajax/libs/jquery/3.7.1/jquery.min.js")
    ...
    /* Function for rendering the dashboard */
        ...
        this.orm.call('vehicle.rental', 'get_dashboard_data', [[]]).then(function(response) {
            mount(OrderStatsTile, $('.cart')[0], {
                props: {
                    total: response.total_cars,
                    pending: response.available_cars,
                    delivered: response.total_drivers,
                    cancelled: response.available_drivers,
                }
            });
        });
