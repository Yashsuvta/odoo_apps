...
class Vehicle(models.Model):
    ...
    # Numeric progress field so progressbar widget can render in kanban
    car_progress = fields.Integer(string='Progress', compute='_compute_car_progress', store=True)
    ...
    def _compute_car_progress(self):
        """Compute a numeric progress value (0-100) for the status.

        The progressbar widget requires a numeric field. We map statuses to
        representative percentages. Adjust the mapping if you prefer other
        values.
        """
        mapping = {
            'available': 0,
        ...
        for rec in self:
            rec.car_progress = mapping.get(rec.car_status, 0)
